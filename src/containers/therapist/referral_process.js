...
class ReferralProcess extends Component {
  ...
  handleRedirectAndToggle = () => {
    this.toggleNavState();
    this.redirect_url('/therapist/member');
  };
    ...
    return (
      <div className="d-flex flex-column therapist-noprogress">
        <div className="d-flex flex-row justify-content-between align-items-center header">
          ...
          <div className="therapist-cover-top-menu desktop-only" onClick={() => this.redirect_url('/therapist/member')}>
            {id ? 'PROFILE' : 'Member Login'}
          </div>
        </div>
        <div className={cx('nav-menu', this.state.showNav ? 'visible' : 'hidden')}>
          <div className={cx('nav-menu--item d-flex justify-content-center align-items-center  profile-side-item')} onClick={this.handleRedirectAndToggle}>
            {id ? 'PROFILE' : 'Member Login'}
          </div>
        </div>

        <div className="d-flex flex-column justify-content-center align-self-center therapist-referral-process__cover">
          ...
          <div className="d-flex flex-row justify-content-between align-items-start therapist-referral-process__cover__options">
            <div className="col d-flex flex-column justify-content-center therapist-referral-process__cover__options__option">
  ...
  type_to_view = actions => {
    switch (this.state.view_type) {
      case 'account':
        return <Account next_type="patient_refer" default_type="register" update_type_handler={this.update_type_handler} />;
      case 'signin':
        return <Account next_type="patient_refer" default_type="signin" update_type_handler={this.update_type_handler} />;
      case 'patient_refer':
        return (
          <PatientsRefer
            submit_action={this.patient_refer_handler}
      ...
      case 'patient_info':
        return (
          <PatientInfo
            patients_info={this.state.patients}
            update_type_handler={this.update_type_handler}
  ...
  render() {
    const { therapist_actions } = this.props;
    return this.type_to_view(therapist_actions);
  }
}
