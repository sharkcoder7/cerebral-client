...
class Account extends Component {
  ...
  register_handler = info => {
    ...
    global_actions
      .register_and_set_user(info)
      .then(() => {
        return global_actions.sign_in(info);
      })
      .then(() => {
        return therapist_actions.create_therapist_from_user();
      })
      .then(() => {
        this.props.update_type_handler(this.props.next_type);
      })
  ...
  view = () => {
    const wording = this.state.type === 'register' ? 'Create an account to refer patients' : 'Sign in to refer patients';
    return (
        ...
        <div className="d-flex flex-column">
          ...
          <div className="d-flex flex-column question-container">
            <div className="d-flex flex-column main-noprogress">
              ...
              <RegisterManager
                update_type={this.update_type_handler}
                user_type="therapist"
                signin_submit_action={this.sign_in_handler}
                register_submit_action={this.register_handler}
                view_type={this.props.default_type}
              />
            </div>
          </div>
        </div>
