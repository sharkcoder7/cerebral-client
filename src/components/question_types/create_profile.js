...
class CreateProfile extends Component {
  ...
  update_handler = e => {
    const {
      is_consent, email, first_name, last_name, password, password_confirm
    } = this.state;
    if (is_consent && email && first_name && last_name && password.length >= 6 && password === password_confirm) {
      this.props.submit_action(this.state);
    } else if (!email || !first_name || !last_name || !password || !password_confirm) {
  ...
  render() {
    const btn_wording = this.props.user_type === 'therapist' ? 'Start referring patients' : 'Get started with your online visit';
    const chk_wording = this.props.user_type === 'therapist'
      ? 'I have consent from my patients to share their contact information for a prescription referral and to provide information regarding their treatment plan to the prescribing physician'
      : 'I consent to Telehealth, terms & conditions and have read Cerebral’s privacy policy. All information is strictly confidential and is used to help our professionals provide the best care for you.';
    ...
    return (
      <div className="create-profile">
        ...
        <div className="d-flex flex-row create-profile__input-wrapper">
        ...
        <div className="d-flex flex-row justify-content-start">{components.checkbox_type_1(this.set_concensus.bind(this), chk_wording)}</div>
