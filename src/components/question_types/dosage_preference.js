...
import { get_treatment_dosages } from '../../actions/patient_action';

class DosagePreference extends Component {
  constructor(props) {
    super(props)
    this.state = {
      isselected:'',
  ...
  componentDidMount = () => {
    // TODO: get selected treatment from visit!
    this.props.get_treatment_dosages(1).then((resp) => {
    ...
    return(
      <div className="input-group mb-3" key={uuidv1()}>
        ...
        <div className="d-flex justify-content-center form-control group-checkbox-text">
          <p className="text-small">{item.dosage} milligrams</p>
        </div>
      </div>
      )
    }
...
export default connect(null, {get_treatment_dosages}) (DosagePreference)